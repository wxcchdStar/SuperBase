...
public class AsyncDownloader implements IDownloader, Runnable {
    ...
    public void startDownload() {
        Log.d("AsyncDownloader", "startDownload：" + mDownloadTask.mDownloadUrl);
    ...
    public void run() {
        ...
        try {
            ...
            if (e instanceof DownloadException) {
                Log.d("AsyncDownloader", "下载失败，原因：" + ((DownloadException) e).mCode + ", " + e.getMessage());
    ...
    private void onPreDownload() {
        ...
        mDownloadTask.notifyListener();
        Log.d("AsyncDownloader", "准备下载：" + mDownloadTask.mDownloadUrl);
    }
    ...
    private void onDownloading() throws DownloadException {
        ...
        if (downloadFile.exists()) { // 文件存在，断点续传
            ...
            if (parentFile != null && !parentFile.exists()) {
                synchronized (AsyncDownloader.class) {
                    ...
                    if (!mkDirsSuccess) { // 创建目录失败
                        Log.e("AsyncDownloader", "创建目录失败，" + parentFile.getPath());
            ...
            try {
                ...
                e.printStackTrace();
                Log.d("AsyncDownloader", downloadFile.getAbsolutePath() + "-文件创建失败，原因：" + e.getLocalizedMessage());
        ...
        try {
            ...
            while (!isConnected) {
                try {
                    ...
                    // 链接超时
                    retry++;
                    Log.w("AsyncDownloader", String.format(Locale.ENGLISH, "连接超时，第%1$d次尝试，URL:%2$s", retry, mDownloadTask.mDownloadUrl));
            ...
            if (status < HttpURLConnection.HTTP_BAD_REQUEST) { // 小于400认为是正常
                ...
                Log.d("AsyncDownloader", "连接成功，开始下载数据：" + mDownloadTask.mDownloadUrl);
                ...
                while ((rlen = in.read(buffer)) != -1) {
                    ...
                    if (System.currentTimeMillis() - stepTime > TIME_STEP) {
                        ...
                        mDownloadTask.notifyListener();
                        Log.d("AsyncDownloader", "下载中，进度：" + mDownloadTask.mProgress);
                    }
...
//                    Thread.sleep(1000);
                ...
                Log.d("AsyncDownloader", "HTTP错误：" + status);
    ...
    private void onPostDownload() {
        ...
        Log.d("AsyncDownloader", "下载完成时间：" + (mConsumeTime / 1000.0f) + "s");
    }
